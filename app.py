...
DEFAULT_AE_LINK_WINDOW_DAYS = 14
DEFAULT_DV_LINK_WINDOW_DAYS = 30
DEFAULT_DV_MATCH_TERMS = ["PROHIBITED MEDICATION", "PROHIBITED CONCOMITANT MEDICATION", "DISALLOWED MEDICATION", "RESTRICTED MEDICATION"]
...
def is_partial_iso_date(value) -> bool:
    if pd.isna(value):
        return False
    value = str(value).strip()
    return bool(re.fullmatch(r"\d{4}|\d{4}-\d{2}", value))

def configured_rule_parameter(rules: pd.DataFrame, rule_type: str, default=None):
    if rules is None or rules.empty:
        return default
    rows = rules[
        rules["ENABLED"].astype(str).str.upper().isin(["Y", "YES", "TRUE", "1"])
        & rules["RULE_TYPE"].astype(str).str.upper().eq(rule_type.upper())
    ]
    if rows.empty:
        return default
    value = rows.iloc[0].get("PARAMETER", default)
    return default if pd.isna(value) or str(value).strip() == "" else value

def split_config_terms(value, defaults):
    if value is None or (isinstance(value, float) and pd.isna(value)) or str(value).strip() == "":
        return list(defaults)
    return [x.strip().upper() for x in re.split(r"[|;]", str(value)) if x.strip()]
    ...
    if ext in [".xpt", ".xport", ".sas7bdat"]:
        ...
        if hasattr(file_or_path, "read") and not isinstance(file_or_path, (str, os.PathLike)):
            ...
            try:
                ...
                LOGGER.warning("Unable to reset uploaded dataset stream position: %s", exc)
...
AI_READY_METADATA_COLUMNS = ["EVIDENCE_REFERENCE", "AI_CONFIDENCE", "REVIEW_STATUS", "APPROVED_BY", "VERSION"]
...
def standardize_rules(rules: pd.DataFrame) -> pd.DataFrame:
    ...
    if "ACTION" not in rules.columns:
        rules["ACTION"] = ""
    for col in RULE_COLUMNS + AI_READY_METADATA_COLUMNS:
        if col not in rules.columns:
            rules[col] = ""
    # Preserve optional governance metadata for review/export while execution functions use named core fields.
    return rules[RULE_COLUMNS + AI_READY_METADATA_COLUMNS].fillna("")
...
def run_data_validation(data: dict) -> pd.DataFrame:
    ...
    for dom in REQUIRED_DOMAINS:
        ...
        for date_col in DATE_COLS.get(dom, []):
            if date_col in df.columns:
                ...
                partial = nonblank & raw.map(is_partial_iso_date)
                ...
                results.append(_result(f"{dom.upper()}P{date_col}", dom, f"{date_col} partial-date detection", "Warning", "Warning" if partial.any() else "Pass", f"{partial.sum()} partial ISO 8601 values require approved imputation/manual review." if partial.any() else "No partial ISO 8601 values detected.", partial.sum(), "Do not silently impute partial dates; resolve or document an approved handling rule." if partial.any() else ""))
        ...
        if dom in date_pairs:
            ...
            if start in df.columns and end in df.columns:
                ...
                results.append(_result(f"{dom.upper()}006", dom, "Start date is not after end date", "Warning", "Warning" if reversed_dates.any() else "Pass", f"{reversed_dates.sum()} records have {start} after {end}. These records will be excluded from date-dependent rules." if reversed_dates.any() else "Date order is valid.", reversed_dates.sum(), "Correct the affected source dates. Valid records will continue through review." if reversed_dates.any() else ""))
    ...
    if not cm.empty:
        if "CMENDTC" in cm.columns:
            end_blank = cm["CMENDTC"].isna() | cm["CMENDTC"].astype(str).str.strip().isin(["", "NAN", "<NA>"])
            ongoing = cm.get("CMONGO", pd.Series("", index=cm.index)).astype(str).str.upper().isin(["Y", "YES", "ONGOING", "1", "TRUE"])
            unresolved = end_blank & ~ongoing
            results.append(_result("CMONGO001", "cm", "Ongoing medication end-date handling", "Info", "Warning" if ongoing.any() else "Pass", f"{ongoing.sum()} records are explicitly ongoing and will be carried through the configured review cutoff." if ongoing.any() else "No explicitly ongoing medications detected.", ongoing.sum(), "Confirm the study data-cutoff date used for ongoing exposure." if ongoing.any() else ""))
            results.append(_result("CMENDMISS001", "cm", "Missing CMENDTC without ongoing indicator", "Warning", "Warning" if unresolved.any() else "Pass", f"{unresolved.sum()} records have unresolved missing end dates and will be marked Not Evaluable for date-dependent rules." if unresolved.any() else "No unresolved missing CMENDTC values detected.", unresolved.sum(), "Resolve CMENDTC or populate the approved ongoing indicator." if unresolved.any() else ""))
    ...
    if isinstance(prohibited, pd.DataFrame) and not prohibited.empty and "CMDECOD" in prohibited.columns:
        dup = prohibited.duplicated("CMDECOD", keep=False)
        conflict_count = 0
        if "PROTOCOL RATIONALE" in prohibited.columns:
            conflict_count = int((prohibited.groupby("CMDECOD")["PROTOCOL RATIONALE"].nunique(dropna=True) > 1).sum())
        results.append(_result("CTPROH001", "", "Prohibited medication list uniqueness", "Warning", "Warning" if dup.any() else "Pass", f"{dup.sum()} duplicate rows across {conflict_count} medication(s) with conflicting rationale." if dup.any() else "No duplicate prohibited-medication terms detected.", dup.sum(), "Reconcile duplicate terms and protocol-amendment rationale before approval." if dup.any() else ""))

    return pd.DataFrame(results)


def build_date_query_report(data: dict) -> pd.DataFrame:
    """Return actionable record-level date issues without stopping valid records."""
    rows = []
    configs = {
        "cm": ("CMSTDTC", "CMENDTC", "CMSEQ", "Medication"),
        "ex": ("EXSTDTC", "EXENDTC", "EXSEQ", "Exposure"),
        "ae": ("AESTDTC", "AEENDTC", "AESEQ", "Adverse Event"),
        "dv": ("DVDTC", None, "DVSEQ", "Protocol Deviation"),
    }
    for dom, (start_col, end_col, seq_col, label) in configs.items():
        df = data.get(dom, pd.DataFrame())
        if not isinstance(df, pd.DataFrame) or df.empty:
            continue
        work = df.copy()
        for col in ["STUDYID", "USUBJID", seq_col, start_col] + ([end_col] if end_col else []):
            if col and col not in work.columns:
                work[col] = pd.NA
        start_raw = work[start_col]
        start_nonblank = start_raw.notna() & ~start_raw.astype(str).str.strip().isin(["", "NAN", "<NA>"])
        start_dt = pd.to_datetime(start_raw, errors="coerce")
        invalid_start = start_nonblank & start_dt.isna()
        for idx in work.index[invalid_start]:
            rows.append({
                "DOMAIN": dom.upper(), "STUDYID": work.at[idx, "STUDYID"], "USUBJID": work.at[idx, "USUBJID"],
                "RECORD_KEY": work.at[idx, seq_col], "VARIABLE": start_col, "ORIGINAL_VALUE": work.at[idx, start_col],
                "ISSUE": "Invalid date value", "SEVERITY": "Warning",
                "RULE_IMPACT": "Date-dependent rules skipped for this record",
                "SUGGESTED_RESOLUTION": "Verify the source value and provide a valid ISO 8601 date or an approved partial-date representation.",
            })
        if end_col:
            end_raw = work[end_col]
            end_nonblank = end_raw.notna() & ~end_raw.astype(str).str.strip().isin(["", "NAN", "<NA>"])
            end_dt = pd.to_datetime(end_raw, errors="coerce")
            invalid_end = end_nonblank & end_dt.isna()
            for idx in work.index[invalid_end]:
                rows.append({
                    "DOMAIN": dom.upper(), "STUDYID": work.at[idx, "STUDYID"], "USUBJID": work.at[idx, "USUBJID"],
                    "RECORD_KEY": work.at[idx, seq_col], "VARIABLE": end_col, "ORIGINAL_VALUE": work.at[idx, end_col],
                    "ISSUE": "Invalid date value", "SEVERITY": "Warning",
                    "RULE_IMPACT": "Date-dependent rules skipped for this record",
                    "SUGGESTED_RESOLUTION": "Verify the source value and provide a valid ISO 8601 date or an approved partial-date representation.",
                })
            reversed_mask = start_dt.notna() & end_dt.notna() & (start_dt > end_dt)
            for idx in work.index[reversed_mask]:
                rows.append({
                    "DOMAIN": dom.upper(), "STUDYID": work.at[idx, "STUDYID"], "USUBJID": work.at[idx, "USUBJID"],
                    "RECORD_KEY": work.at[idx, seq_col], "VARIABLE": f"{start_col}/{end_col}",
                    "ORIGINAL_VALUE": f"{work.at[idx, start_col]} → {work.at[idx, end_col]}",
                    "ISSUE": "Start date occurs after end date", "SEVERITY": "Critical Record Issue",
                    "RULE_IMPACT": "This record is excluded from overlap, duration, washout, and timeline rules",
                    "SUGGESTED_RESOLUTION": "Verify and correct the source start/end dates. Other valid records will continue processing.",
                })
    columns = ["DOMAIN", "STUDYID", "USUBJID", "RECORD_KEY", "VARIABLE", "ORIGINAL_VALUE", "ISSUE", "SEVERITY", "RULE_IMPACT", "SUGGESTED_RESOLUTION"]
...
def apply_metadata_medication_rules(review: pd.DataFrame, rules: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, rule in enabled.iterrows():
        ...
        mask = mask & out.get("RULE_ELIGIBLE", pd.Series("Y", index=out.index)).eq("Y")
...
def evaluate_study_rules(review: pd.DataFrame, data: dict, rules: pd.DataFrame):
    ...
    for _, rule in enabled.iterrows():
        ...
        for idx, row in review.iterrows():
            if str(row.get("RULE_ELIGIBLE", "Y")) != "Y":
                continue
            if rtype == "PROHIBITED_OVERLAP" and row.get("PROHMEDFL") == "Y" and row.get("OVERLAPFL") == "Y":
                ...
                cm_end = row.get("CMENDTC_ANALYSIS_DT")
                trt_start = row.get("TRTSDT")
                try:
...
def prepare_review_dataset(data, rules):
    ...
    if "CMONGO" not in cm.columns:
        cm["CMONGO"] = ""
    cm_start_raw = cm["CMSTDTC"]
    cm_end_raw = cm["CMENDTC"]
    cm["CMSTDTC_DT"] = safe_date(cm, "CMSTDTC")
    cm["CMENDTC_DT"] = safe_date(cm, "CMENDTC")
    start_nonblank = cm_start_raw.notna() & ~cm_start_raw.astype(str).str.strip().isin(["", "NAN", "<NA>"])
    end_nonblank = cm_end_raw.notna() & ~cm_end_raw.astype(str).str.strip().isin(["", "NAN", "<NA>"])
    cm["PARTIAL_CMSTDTC_FL"] = cm_start_raw.map(is_partial_iso_date).map({True: "Y", False: "N"})
    cm["PARTIAL_CMENDTC_FL"] = cm_end_raw.map(is_partial_iso_date).map({True: "Y", False: "N"})
    cm["INVALID_CMSTDTC_FL"] = (start_nonblank & cm["CMSTDTC_DT"].isna()).map({True: "Y", False: "N"})
    cm["INVALID_CMENDTC_FL"] = (end_nonblank & cm["CMENDTC_DT"].isna()).map({True: "Y", False: "N"})
    cm["REVERSED_CM_DATE_FL"] = (cm["CMSTDTC_DT"].notna() & cm["CMENDTC_DT"].notna() & (cm["CMSTDTC_DT"] > cm["CMENDTC_DT"])).map({True: "Y", False: "N"})
    cm["CM_IS_ONGOING"] = cm["CMONGO"].astype(str).str.upper().isin(["Y", "YES", "ONGOING", "1", "TRUE"])
    cm["MISSING_END_UNRESOLVED_FL"] = ((~end_nonblank) & ~cm["CM_IS_ONGOING"]).map({True: "Y", False: "N"})
    cm["DATE_ISSUES"] = ""
    cm.loc[cm["INVALID_CMSTDTC_FL"].eq("Y"), "DATE_ISSUES"] += "Invalid CMSTDTC; "
    cm.loc[cm["INVALID_CMENDTC_FL"].eq("Y"), "DATE_ISSUES"] += "Invalid CMENDTC; "
    cm.loc[cm["PARTIAL_CMSTDTC_FL"].eq("Y"), "DATE_ISSUES"] += "Partial CMSTDTC; "
    cm.loc[cm["PARTIAL_CMENDTC_FL"].eq("Y"), "DATE_ISSUES"] += "Partial CMENDTC; "
    cm.loc[cm["REVERSED_CM_DATE_FL"].eq("Y"), "DATE_ISSUES"] += "CMSTDTC after CMENDTC; "
    cm.loc[cm["MISSING_END_UNRESOLVED_FL"].eq("Y"), "DATE_ISSUES"] += "Missing CMENDTC without ongoing indicator; "
    cm["DATE_ISSUES"] = cm["DATE_ISSUES"].str.rstrip("; ")
    blocking_cols = ["INVALID_CMSTDTC_FL", "INVALID_CMENDTC_FL", "PARTIAL_CMSTDTC_FL", "PARTIAL_CMENDTC_FL", "REVERSED_CM_DATE_FL"]
    cm["RULE_ELIGIBLE"] = (~cm[blocking_cols].eq("Y").any(axis=1)).map({True: "Y", False: "N"})
    cm["DATA_QUALITY_STATUS"] = cm["RULE_ELIGIBLE"].map({"Y": "Ready for automated review", "N": "Needs data correction"})
    ...
    review["WINDOW_CONFIG_STATUS"] = "Configured" if window_days is not None else "Missing - date overlap not evaluable"
    review["RESTRICT_END"] = review["TRTEDT"] + pd.Timedelta(days=int(window_days)) if window_days is not None else pd.NaT
    study_cutoff = max_date(pd.concat([
        review.get("TRTEDT", pd.Series(dtype="datetime64[ns]")),
        review.get("CMSTDTC_DT", pd.Series(dtype="datetime64[ns]")),
        review.get("CMENDTC_DT", pd.Series(dtype="datetime64[ns]")),
    ], ignore_index=True))
    if pd.isna(study_cutoff):
        study_cutoff = pd.Timestamp.today().normalize()
    review["CMENDTC_ANALYSIS_DT"] = review["CMENDTC_DT"]
    review.loc[review["CM_IS_ONGOING"].fillna(False) & review["CMENDTC_ANALYSIS_DT"].isna(), "CMENDTC_ANALYSIS_DT"] = study_cutoff
    review["CM_END_DATE_STATUS"] = "COMPLETE"
    review.loc[review["CM_IS_ONGOING"].fillna(False), "CM_END_DATE_STATUS"] = "ONGOING"
    review.loc[review["MISSING_END_UNRESOLVED_FL"].eq("Y"), "CM_END_DATE_STATUS"] = "MISSING_UNRESOLVED"
    review["OVERLAPFL"] = "N"
    unknown_overlap = review["RULE_ELIGIBLE"].ne("Y") | review["RESTRICT_END"].isna() | review["CMSTDTC_DT"].isna() | review["CMENDTC_ANALYSIS_DT"].isna() | review["MISSING_END_UNRESOLVED_FL"].eq("Y")
    review.loc[unknown_overlap, "OVERLAPFL"] = "U"
    overlap = (~unknown_overlap) & (review["CMSTDTC_DT"] <= review["RESTRICT_END"]) & (review["CMENDTC_ANALYSIS_DT"] >= review["RESTRICT_START"])
    review.loc[overlap, "OVERLAPFL"] = "Y"
    ...
    if not prohibited.empty and "CMDECOD" in prohibited.columns:
        ...
        p_source = prohibited[merge_cols].copy()
        rationale_col = "PROTOCOL RATIONALE" if "PROTOCOL RATIONALE" in p_source.columns else None
        if rationale_col:
            conflict_counts = p_source.groupby("CMDECOD")[rationale_col].nunique(dropna=True)
            conflicts = set(conflict_counts[conflict_counts > 1].index.astype(str))
        else:
            conflicts = set()
        p2 = p_source.drop_duplicates("CMDECOD", keep="first").copy()
        p2["PROTOCOL_LIST_CONFLICT_FL"] = p2["CMDECOD"].astype(str).isin(conflicts).map({True:"Y", False:"N"})
    ...
    # AE linkage uses medication-specific temporal logic.
    ae_window = int(float(configured_rule_parameter(rules, "AE_LINK_WINDOW_DAYS", DEFAULT_AE_LINK_WINDOW_DAYS)))
    review["AE_COUNT"] = 0
    review["SERIOUS_AE_COUNT"] = 0
    review["AE_LINKED_COUNT"] = 0
    review["AE_LINK_REASON"] = ""
    review["MAX_AE_SEV"] = ""
    if not ae.empty and "USUBJID" in ae.columns:
        ...
        for idx, med in review.iterrows():
            subj_ae = ae[ae["USUBJID"].astype(str).eq(str(med.get("USUBJID", "")))].copy()
            review.at[idx, "AE_COUNT"] = len(subj_ae)
            if subj_ae.empty:
                continue
            serious = subj_ae["AESER"].astype(str).str.upper().eq("Y")
            review.at[idx, "SERIOUS_AE_COUNT"] = int(serious.sum())
            review.at[idx, "MAX_AE_SEV"] = ", ".join(sorted(set(subj_ae["AESEV"].dropna().astype(str))))[:80]
            cm_start = med.get("CMSTDTC_DT")
            if pd.notna(cm_start):
                delta = (subj_ae["AESTDTC_DT"] - cm_start).dt.days
                within = delta.between(-ae_window, ae_window, inclusive="both")
            else:
                within = pd.Series(False, index=subj_ae.index)
            linked = within | serious
            review.at[idx, "AE_LINKED_COUNT"] = int(linked.sum())
            reasons = []
            if within.any(): reasons.append(f"AE within ±{ae_window} days")
            if serious.any(): reasons.append("Serious AE")
            review.at[idx, "AE_LINK_REASON"] = "; ".join(reasons)
    review["AE_COUNT"] = pd.to_numeric(review["AE_COUNT"], errors="coerce").fillna(0).astype(int)
    review["SERIOUS_AE_COUNT"] = pd.to_numeric(review["SERIOUS_AE_COUNT"], errors="coerce").fillna(0).astype(int)
    review["AEREL_REVIEWFL"] = ((review["AE_LINKED_COUNT"] > 0) & review["PROHMEDFL"].eq("Y")).map({True:"Y", False:"N"})

    # DV reconciliation uses configurable phrases and record-level timing.
    dv_terms = split_config_terms(configured_rule_parameter(rules, "DV_MATCH_TERMS", None), DEFAULT_DV_MATCH_TERMS)
    dv_window = int(float(configured_rule_parameter(rules, "DV_LINK_WINDOW_DAYS", DEFAULT_DV_LINK_WINDOW_DAYS)))
    ...
    if not dv.empty and "USUBJID" in dv.columns:
        ...
        normalized = (dv["DVCAT"].fillna("").astype(str) + " " + dv["DVTERM"].fillna("").astype(str)).str.upper().str.replace(r"\s+", " ", regex=True)
        dv["IS_PROH_DV"] = False
        for term in dv_terms:
            dv["IS_PROH_DV"] = dv["IS_PROH_DV"] | normalized.str.contains(re.escape(term), regex=True, na=False)
        for idx, med in review.iterrows():
            subj = dv[dv["USUBJID"].astype(str).eq(str(med.get("USUBJID", ""))) & dv["IS_PROH_DV"]].copy()
            if subj.empty:
                continue
            cm_start = med.get("CMSTDTC_DT")
            if pd.notna(cm_start):
                delta = (subj["DVDTC_DT"] - cm_start).dt.days
                subj = subj[delta.between(-dv_window, dv_window, inclusive="both")]
            if subj.empty:
                continue
            review.at[idx, "DV_PROH_COUNT"] = len(subj)
            review.at[idx, "DV_IMPORTANT_COUNT"] = int(subj["IMPORTANT"].astype(str).str.upper().isin(["Y", "YES", "IMPORTANT"]).sum())
            review.at[idx, "FIRST_DVDTC"] = subj["DVDTC_DT"].min()
    review["DV_PROH_COUNT"] = pd.to_numeric(review["DV_PROH_COUNT"], errors="coerce").fillna(0).astype(int)
    review["DV_IMPORTANT_COUNT"] = pd.to_numeric(review["DV_IMPORTANT_COUNT"], errors="coerce").fillna(0).astype(int)
    ...
    review["IPD_FL"] = "N"
    review.loc[(review["PROHMEDFL"] == "Y") & (review["OVERLAPFL"] == "Y"), "IPD_FL"] = "Y"
    review.loc[(review["PROHMEDFL"] == "Y") & (review["OVERLAPFL"] == "U"), "IPD_FL"] = "U"
    review["REVIEW_STATUS"] = "No Action"
    review.loc[(review["PROHMEDFL"] == "Y") & (review["OVERLAPFL"] == "N"), "REVIEW_STATUS"] = "Timing Review"
    review.loc[(review["PROHMEDFL"] == "Y") & (review["OVERLAPFL"] == "U"), "REVIEW_STATUS"] = "Manual Review / Not Evaluable"
    ...
    review.loc[review.get("RULE_ELIGIBLE", pd.Series("Y", index=review.index)).ne("Y"), "REVIEW_STATUS"] = "Needs Data Correction"

    review["RISK_LEVEL"] = "Low"
    review.loc[review["PROHMEDFL"] == "Y", "RISK_LEVEL"] = "Medium"
    review.loc[review["IPD_FL"] == "Y", "RISK_LEVEL"] = "High"
    review.loc[(review["IPD_FL"] == "Y") & (review["AEREL_REVIEWFL"] == "Y"), "RISK_LEVEL"] = "Critical"
    review.loc[review["IPD_FL"].eq("U"), "RISK_LEVEL"] = "Manual Review"
...
st.markdown(
    ...
    <style>
      ...
      .launch-card-title { color: #0F4C81; font-weight: 750; font-size: 0.88rem; }
      .launch-card-copy { color: #475569; font-size: 0.78rem; margin-top: 0.2rem; }
      .module-pill {
    ...
    <div class="clinexa-shell">
      ...
      <div class="module-pill">Clinical Review Application · v1.2</div>
      <div class="launch-grid">
        <div class="launch-card"><div class="launch-card-title">Data Quality Firewall</div><div class="launch-card-copy">Identify critical data issues before clinical-rule execution.</div></div>
        <div class="launch-card"><div class="launch-card-title">Rule Readiness</div><div class="launch-card-copy">Confirm domains, variables, parameters, and compatibility.</div></div>
        <div class="launch-card"><div class="launch-card-title">AI Rule Builder</div><div class="launch-card-copy">Planned: generate governed draft rules from study documents.</div></div>
      </div>
    </div>
    """,
...
with st.sidebar:
    st.markdown("### Clinexa AI · Study Setup")
    st.caption("Follow the guided setup. Uploaded files override bundled dummy content for the current session.")
    ...
    with st.expander("3. Rule Source", expanded=True):
        st.markdown("**Select how study rules are supplied**")
        st.markdown("✅ **Upload Existing Rule Workbook**")
        ...
        st.button("Generate Using AI · Coming Soon", disabled=True, use_container_width=True, help="Planned capability: generate draft rules from Protocol, SAP, medication guidance, and licensed dictionary extracts.")
        st.caption("AI-generated rules will remain drafts until reviewed and approved. TrailGuard execution stays deterministic.")
        if os.path.exists(RULES_XLSX_PATH):
...
try:
    ...
    window_rule = rules[
        rules["ENABLED"].astype(str).str.upper().isin(["Y", "YES", "TRUE", "1"])
        & rules["RULE_TYPE"].astype(str).str.upper().eq("POST_TREATMENT_WINDOW_DAYS")
    ]
    data["window_days"] = None
    data["window_config_warning"] = "POST_TREATMENT_WINDOW_DAYS is missing; overlap and IPD timing are Not Evaluable."
    if not window_rule.empty:
        try:
            data["window_days"] = int(float(window_rule.iloc[0]["PARAMETER"]))
            data["window_config_warning"] = ""
        except Exception as exc:
            LOGGER.warning("Invalid POST_TREATMENT_WINDOW_DAYS parameter: %s", exc)
            data["window_config_warning"] = "POST_TREATMENT_WINDOW_DAYS is invalid; overlap and IPD timing are Not Evaluable."
    validation_results = run_data_validation(data)
    if data.get("window_config_warning"):
        validation_results = pd.concat([validation_results, pd.DataFrame([_result("CFGWIN001", "", "Post-treatment window configured", "Warning", "Warning", data["window_config_warning"], 1, "Configure and approve POST_TREATMENT_WINDOW_DAYS in the Study Rule Workbook.")])], ignore_index=True)
    date_query_report = build_date_query_report(data)
    ...
    if "READINESS" in error_text:
        ...
        st.write("Open **Rule Source**, upload the Clinexa rule workbook, and then return to **Rule Readiness & Compatibility**.")
...
if info.get("error"):
    st.info("### Next step: Upload a CM dataset")
    st.write("Open **Dataset Upload** and provide a non-empty CM dataset in CSV, XPT/XPORT, SAS7BDAT, or a ZIP package. Then return to the Data Validation Center.")
    st.stop()

# Concise study setup summary keeps the workflow understandable without adding configuration burden.
loaded_domains = sum(1 for dom in REQUIRED_DOMAINS if isinstance(data.get(dom), pd.DataFrame) and not data.get(dom).empty)
enabled_rules = int(rules["ENABLED"].astype(str).str.upper().isin(["Y", "YES", "TRUE", "1"]).sum()) if not rules.empty else 0
protocol_status = "Uploaded" if protocol_file is not None else "Bundled Demo"
sap_status = "Uploaded" if sap_file is not None else "Bundled Demo"
rule_status = "Uploaded" if rules_file is not None else "Bundled Demo"
summary_cols = st.columns(6)
summary_values = [
    ("Protocol", protocol_status),
    ("SAP", sap_status),
    ("Rule Source", rule_status),
    ("Enabled Rules", enabled_rules),
    ("Loaded Domains", f"{loaded_domains}/{len(REQUIRED_DOMAINS)}"),
    ("Execution", "Blocked" if critical_validation_block else "Ready"),
]
...
with c8: render_metric("Needs Data Correction", int((review.get("RULE_ELIGIBLE", pd.Series("Y", index=review.index)) != "Y").sum()))
...
with tab2:
    ...
    display_cols = [
        ...
        "CMSEQ", "CMTRT", "CMDECOD", *ATC_ALL_COLUMNS, "CMSTDTC", "CMENDTC", "CMONGO", "CM_END_DATE_STATUS", "CMENDTC_ANALYSIS_DT",
        "DATA_QUALITY_STATUS", "DATE_ISSUES", "RULE_ELIGIBLE",
        ...
        "AE_COUNT", "AE_LINKED_COUNT", "AE_LINK_REASON", "AEREL_REVIEWFL", "SERIOUS_AE_COUNT", "DV_RECON_STATUS", "REVIEW_STATUS",
...
with tab5:
    ...
    with ch1: st.metric("Treatment Window", f"{data.get('window_days')} days" if data.get('window_days') is not None else "Not Configured")
    with ch2: st.metric("Partial Dates", int(validation_results[validation_results["CHECK"].str.contains("partial-date", case=False, na=False)]["AFFECTED_RECORDS"].sum()))
    with ch3: st.metric("Ongoing CM", int(validation_results.loc[validation_results["CHECK_ID"].eq("CMONGO001"), "AFFECTED_RECORDS"].sum()))
    with ch4: st.metric("Unresolved End Dates", int(validation_results.loc[validation_results["CHECK_ID"].eq("CMENDMISS001"), "AFFECTED_RECORDS"].sum()))
    if data.get("window_config_warning"):
        st.warning(data["window_config_warning"])

    st.markdown("### 🛡 Data Quality Firewall")
    total_review_records = len(review)
    excluded_records = int((review.get("RULE_ELIGIBLE", pd.Series("Y", index=review.index)) != "Y").sum())
    valid_records = total_review_records - excluded_records
    q1, q2, q3 = st.columns(3)
    with q1: st.metric("Ready for Automated Review", valid_records)
    with q2: st.metric("Needs Data Correction", excluded_records)
    with q3: st.metric("Date Queries", len(date_query_report))

    if not date_query_report.empty:
        st.warning(f"{len(date_query_report)} date issue(s) were identified. Affected records are excluded only from date-dependent rules; valid records continue processing.")
        st.dataframe(date_query_report, use_container_width=True, hide_index=True)
        st.download_button(
            "Download date validation query report",
            date_query_report.to_csv(index=False).encode("utf-8"),
            file_name="clinexa_date_validation_queries.csv",
            mime="text/csv",
        )
        st.info("Next step: Send the downloaded query report to Data Management, correct the source dates, and upload the refreshed dataset. No action is required for valid records.")
    else:
        st.success("No invalid or reversed date records were detected.")

    st.markdown("**Records requiring clinical/data review**")
    review_subset = review[(review.get("RULE_ELIGIBLE", pd.Series("Y", index=review.index)) != "Y") | ((review["PROHMEDFL"] == "Y") & ((review["OVERLAPFL"] == "N") | (review["DV_MATCHFL"] == "N")))]
    st.dataframe(review_subset[[c for c in ["USUBJID", "CMSEQ", "CMTRT", "CMDECOD", "CMSTDTC", "CMENDTC", "DATA_QUALITY_STATUS", "DATE_ISSUES", "RULE_ELIGIBLE", "OVERLAPFL", "IPD_FL", "DV_RECON_STATUS", "REVIEW_STATUS"] if c in review_subset.columns]], use_container_width=True, hide_index=True)
...
with tab6:
    ...
    if rule_readiness.empty:
        ...
        st.write("Go to **3. Rule Source**, upload the Clinexa Excel rule workbook or CSV rule file, and then return to this tab.")
...
st.caption(f"Clinexa AI · TrialGuard v{APP_VERSION} | Demonstration dashboard using dummy oncology SDTM-like data. Medical review and protocol interpretation must confirm final Important Protocol Deviation decisions. AI Rule Builder is a planned, human-governed capability.")
